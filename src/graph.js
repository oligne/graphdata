...
export function applyLayoutForces(graph, mode = 'arbo') {
  ...
  if (mode === 'year') {
    ...
    graph.d3Force('y', forceY(0).strength(0.05)); // aligne sur une ligne horizontale
    graph.d3Force('z', forceZ(0).strength(0.02)); // léger aplatissement
  } else {
    // Arbo / Dossiers: centrage doux
    graph.d3Force('x', forceX(0).strength(0.02));
